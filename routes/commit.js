...
function forkWhale( parent, res ){

    var whalesModel = new Whales();
    whalesModel.init_commit( parent );
...
function forkPage( parent, res ){

    var pageModel = new Pages();
    pageModel.init_commit( parent );
                ...
                if( err ){
                    ...
                    Whales.findOne( {uuid:req.body.uuid, backup_num : 0 }, function(err, whale){
                        if( err ) {
                            ...
                            // backup
                            whale.do_commit( function( err, doc ){
                                if( err ){
                ...
                if( err ){
                    ...
                    Pages.findOne( {uuid:req.body.uuid, backup_num : 0 }, function(err, page){
                        if( err ) {
                            ...
                            // backup
                            page.do_commit( function( err, doc ){
                                if( err ){
                                    res.status(ERROR_COMMIT_WHALES_INTERNAL2).send(err);
                                }else{
                                    forkPage( page, res );
